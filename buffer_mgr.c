...
RC unpinPage (BM_BufferPool *const bm, BM_PageHandle *const page)
{
    ...
    if (bm->mgmtData != NULL) 
    {
        ...
        // get the mapped framedIndex from pageNum
        int getValueResult = getValue(pageTable, page->pageNum, &framedIndex);
        switch (getValueResult) 
        {
            case 0:
                ...
                // decrement fixedCount but ensure it does not drop below 0
...
RC pinPage (BM_BufferPool *const bm, BM_PageHandle *const page, const PageNumber pageNum)
{
    ...
    switch (bm->mgmtData != NULL) 
    {
                        ...
                        default:  // Page is not in a frame, use replacement strategy
                            ...
                            switch (bm->strategy)
                            {
                                ...
                                case RS_FIFO:
                                    ...
                                    break;
                                     // Configuration error if no strategy fit
                                default:
                                    return RC_IM_CONFIG_ERROR;
                            }

                            // see if replacement strategy succeeded
                            if (pageFrame == NULL)
        ...
        // Management data not initialized
        default:
            return RC_FILE_HANDLE_NOT_INIT;  
    }
}
